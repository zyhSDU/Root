 ...
 * body of the job's <code>response(..)</code> periodically check some flag to
 ...
 * synchronization within <code>interrupt()</code> and <code>response(..)</code>
 * in order to have the <code>interrupt()</code> method block until the
 * <code>response(..)</code> signals that it has noticed the set flag.
 * </p>
 * 
 * <p>
 * If the Job performs some form of blocking I/O or similar functions, you may
 * want to consider having the <code>Job.response(..)</code> method store a
 ...
 * does and doesn't response.  Also make sure that you clear the Job's member
 * reference to the Thread when the response(..) method exits (preferably in a
